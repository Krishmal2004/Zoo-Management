...
import { getStoreItems, createStoreItem, updateStoreItem, deleteStoreItem } from '../../api/store.api';
import { getAdminDrawerMenuItems, getAdminModuleHeroByRouteName } from './adminNavigation';

const CATEGORIES = ['souvenir', 'clothing', 'food', 'toy', 'book', 'other'];
const BLANK = { name: '', description: '', price: '', category: 'souvenir', imageUrl: '', stock: '', isAvailable: true };
...
export default function AdminOnlineStoreScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const r = await getStoreItems(); setItems(r.data || []); }
    catch { setError('Failed to load store items.'); }
  ...
  const openEdit = (item) => {
    ...
    setDraft({
      ...
      price: String(item.price ?? ''), category: item.category || 'souvenir',
      imageUrl: item.imageUrl || '', stock: String(item.stock ?? ''),
  ...
  const handleSave = async () => {
    if (!draft.name.trim() || !draft.price || draft.stock === '') {
      setFormError('Name, price and stock are required.'); return;
    }
    setSaving(true); setFormError('');
    const payload = { ...draft, price: parseFloat(draft.price), stock: parseInt(draft.stock) };
    try {
      if (editing) { await updateStoreItem(editing._id, payload); }
  ...
  const handleDelete = (item) => {
    Alert.alert('Delete Item', `Delete "${item.name}"?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await deleteStoreItem(item._id); setItems(p => p.filter(x => x._id !== item._id)); }
  ...
  return (
    <AccountDrawerLayout headerTitle="Admin" drawerMenuItems={drawerMenuItems}>
      ...
      <PrimaryButton title="＋ Add Store Item" onPress={openNew} style={styles.addBtn} />
      <TextField label="Search" value={search} onChangeText={setSearch} placeholder="Name or category…" />
        ...
        <View key={item._id} style={styles.card}>
          <View style={styles.cardTop}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.cardMeta}>{item.category} · LKR {item.price} · Stock: {item.stock}</Text>
              {item.description ? <Text style={styles.cardDesc} numberOfLines={2}>{item.description}</Text> : null}
            </View>
            <View style={[styles.statusBadge, !item.isAvailable && styles.statusBadgeOff]}>
              <Text style={[styles.statusText, !item.isAvailable && styles.statusTextOff]}>{item.isAvailable ? 'Available' : 'Hidden'}</Text>
            </View>
          </View>
      ...
      {!loading && filtered.length === 0 && <Text style={styles.hint}>No items found.</Text>}
      ...
      <Modal visible={showModal} animationType="slide">
        <ScrollView contentContainerStyle={styles.modal} keyboardShouldPersistTaps="handled">
          ...
          <TextField label="Name *" value={draft.name} onChangeText={(v) => setF('name', v)} />
          ...
          <TextField label="Stock *" value={draft.stock} onChangeText={(v) => setF('stock', v)} keyboardType="numeric" />
          ...
          <ChipRow options={['available', 'hidden']} selected={draft.isAvailable ? 'available' : 'hidden'} onSelect={(v) => setF('isAvailable', v === 'available')} />
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: theme.colors.white, borderRadius: theme.radii.md, borderWidth: 1, borderColor: theme.colors.border, borderLeftWidth: 4, borderLeftColor: '#ec4899', padding: theme.spacing.md, marginBottom: theme.spacing.sm },
  ...
  cardDesc: { fontSize: theme.fontSize.sm, color: theme.colors.primaryText, opacity: 0.65, marginTop: 4, lineHeight: 18 },
